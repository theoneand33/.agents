...
## Whitelist (Sender Filtering)

A whitelist is configured in `config.json` to block emails from non-whitelisted senders.

**Config location:** `~/.agents/skills/agentmail/config.json`

```json
{
  "whitelist_enabled": true,
  "whitelist_mode": "block_all_except_whitelist",
  "allowed_senders": ["trusted@example.com", "partner@company.org"],
  "allowed_domains": ["trusted-company.com"]
}
```

**Before processing any incoming email:**

1. Read `config.json` to check whitelist settings
2. If `whitelist_enabled` is `true`:
   - Extract the sender email from the message
